...
const Modal = ({ children, onClose, optionalStyles, containerStyles }: ModalProps) => {

    const hoverActiveAnim = "hover:scale-105 active:scale-95 transition-all"

    return (
        <div data-modal className={`absolute inset-0 flex items-center justify-center bg-gray-100/50 dark:bg-gray-950/70 backdrop-blur-[2px] pointer-events-none z-50 ${optionalStyles}`} onKeyDown={(e) => e.key == "Escape" && onClose()}>
