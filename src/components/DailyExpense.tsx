                        ...
                        <ul className="md:w-fit grid justify-items-center content-center gap-2 grid-cols-4 md:grid-cols-7 text-[1.4rem] md:text-[1.75rem] leading-5 md:leading-7">
                            {getDayExpenseSum().map(({ day, total, dayIndex }) => (
                                <button
                                    ...
                                    className={`min-w-[4.75rem] min-h-[4.75rem] md:w-[6.25rem] md:h-[6.25rem] flex items-center justify-center flex-col border-2 rounded-sm font-black stdInt bg-gradient-to-tr ${dayIndex == 0 ? isSunday : dayIndex == 1 ? isMonday : dayIndex == 6 ? isSaturday : isDefault} ${Number(total.toFixed(2)) > 60 ? "text-red-500" : Number(total.toFixed(2)) > 40 ? "text-yellow-200" : "text-green-400"} `}
                                    key={day}>
                                    {total ? (
                                        <>
